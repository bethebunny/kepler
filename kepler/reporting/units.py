...
class Unit(enum.Enum):
    ...
    def __init__(self, shortname: str, base: Optional[int]):
    ...
    def in_nanos(self):
        return math.prod((unit.base or 1) for unit in Unit if unit < self)
    ...
    def format(self, units: int):
        return f"{units}{self.shortname}"


def format_timedelta_ns(nanos: float, precision: int = 3) -> str:
    unit = max(
        (unit for unit in Unit if (nanos // unit.in_nanos)), default=Unit.NANOS
    )
    if unit >= Unit.MINUTES:
        ...
        return f"{unit.format(int(units))}{second.format(int(seconds))}"
